...
def payback_menu(parent_window):
    ...
    window = sg.Window('Calculadora de Payback', layout, finalize=True)
    window.Maximize()
...
def show_optimization(parent_window):
    caminho_imagem = os.path.join(os.getcwd(), 'figura_angulos_ideais.png')
    layout = [
        [sg.Column([[sg.Image(filename=caminho_imagem)]]),
    ...
    info_window = sg.Window("Otimização", layout, modal=True, finalize=True)
